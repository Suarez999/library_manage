...
<div id="MainArea">
	<!-- 111表单内容 -->
	<form action="${pageContext.request.contextPath }/TeacherServlet?method=update" method="post">
		<!-- 本段标题（分段标题） -->
				...
				<div class="ItemBlock2">
					<table cellpadding="0" cellspacing="0" class="mainForm">
						<tr>
							...
							<td><input type="text" name="name" class="InputStyle" value="${requestScope.teacher.name }" readonly="readonly"/> *</td>
						</tr>
                    	<tr>
							<td width="80px">username</td>
							<td>
                            	<input type="text" name="username" class="InputStyle" value="${requestScope.teacher.username }"> *
                            </td>
						</tr>
						<tr>
							<td width="80px">phone</td>
							<td>
	                            <input type="text" name="phone" class="InputStyle" value="${requestScope.teacher.phone }"> *
                            </td>
						</tr>
						<tr>
							<td width="80px">email</td>
							<td>
								<input type="text" name="email" class="InputStyle" value="${requestScope.teacher.email }"/> *
							</td>
						</tr>
						<tr>
							<td width="80px">address</td>
							<td>
								<input type="text" name="address" class="InputStyle" value="${requestScope.teacher.address }"/> *
							</td>
						</tr>
						<tr>
							<td width="80px">class</td>
							<td>
								<input type="text" name="teacher_class" class="InputStyle" value="${requestScope.teacher.teacher_class }"/> *
							</td>
						</tr>
						<tr>
							<td>state</td>
							<c:choose>
								<c:when test="${requestScope.teacher.state==1 }">
									<td>
						...
						<tr>
							<td>fine</td>
							<c:choose>
								<c:when test="${requestScope.teacher.fine==null }">
									<td>
								...
								<c:otherwise>
									<td>
										<input type="text" name="fine" class="InputStyle" value="${requestScope.teacher.fine }"/> *
									</td>
								</c:otherwise>
							</c:choose>
						</tr>
						<tr>
							<td></td>
							<td><input type="hidden" name="id" value="${requestScope.teacher.id }"></td>
						</tr>
					</table>
				</div>
            </div>
