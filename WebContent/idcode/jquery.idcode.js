...
(function($){
	...
	var settings2 = {
			...
			codeType 	: { name : 'follow', len: 5},//len是修改验证码长度的
			codeTip		: 'change?',
			inputID		: 'Txtidcode2'//验证元素的ID
		};
	var settings3 = {
			e	 		: 'idcode3',
			codeType 	: { name : 'follow', len: 5},//len是修改验证码长度的
			codeTip		: 'change?',
			inputID		: 'Txtidcode3'//验证元素的ID
		};
	var _set = {
		storeLable  : 'codeval',
	...
	$.idcode = {
		...
		setCode2:function(option){
			_commSetting(option);
			_setCodeStyle("#"+settings2.e, settings2.codeType.name, settings2.codeType.len);
			
		},
		setCode3:function(option){
			_commSetting(option);
			_setCodeStyle("#"+settings3.e, settings3.codeType.name, settings3.codeType.len);
