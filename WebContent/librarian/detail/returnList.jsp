...
<div id="TitleArea">
	...
	<div id="TitleArea_Title">
		<div id="TitleArea_Title_Content">
			<img border="0" width="13" height="13" src="${pageContext.request.contextPath}/sys/detail/style/images/title_arrow.gif"/> borrow list
		</div>
    </div>
...
<div id="QueryArea">
	<div style="width:600px;float:left;">
			<form action="${pageContext.request.contextPath }/BorrowbookServlet" method="get">
				<input type="hidden" name="method" value="getBorrowBooks">
				<c:choose>
		...
		<div style="width:200px;float:right;">
			<c:choose>
				<c:when test="${requestScope.pb.keyWord!=null }">
					<a href="${pageContext.request.contextPath }/BorrowbookServlet?currentPage=${requestScope.pb.currentPage-1}&&method=getBorrowBooks&&keyWord=${pb.keyWord }">last page</a>
				</c:when>
				<c:otherwise>
					<a href="${pageContext.request.contextPath }/BorrowbookServlet?currentPage=${requestScope.pb.currentPage-1}&&method=getBorrowBooks">last page</a>
				</c:otherwise>
			</c:choose>
			${requestScope.pb.currentPage }/${requestScope.pb.totalPage }
			<c:choose>
				<c:when test="${requestScope.pb.keyWord!=null }">
					<a href="${pageContext.request.contextPath }/BorrowbookServlet?currentPage=${requestScope.pb.currentPage+1}&&method=getBorrowBooks&&keyWord=${pb.keyWord }">next page</a>
				</c:when>
				<c:otherwise>
					<a href="${pageContext.request.contextPath }/BorrowbookServlet?currentPage=${requestScope.pb.currentPage+1}&&method=getBorrowBooks">next page</a>
				</c:otherwise>
			</c:choose>
		</div>
</div>
        ...
        <tbody id="TableData">
			<c:forEach items="${pb.pageData }" var="item">
				<tr class="TableDetail1">
					...
					<td>
						&nbsp;&nbsp;&nbsp;<a href="${pageContext.request.contextPath }/BorrowbookServlet?method=returnBook&&id=${item.id }&&userName=${item.username}&&book_id=${item.book_id }" class="FunctionButton">allow</a>				
						...
						&nbsp;&nbsp;&nbsp;&nbsp;<a href="${pageContext.request.contextPath }/BorrowbookServlet?method=lost&&id=${item.id}&&userName=${item.username}&&book_id=${item.book_id }" class="FunctionButton"><font color="red">lost?</font></a>								
					</td>
				</tr>
			</c:forEach>
        </tbody>
